...
export const useHapticFeedback = () => {
  ...
  const triggerHapticFeedback = useCallback((pattern: VibratePattern = 100) => {
    // Default to true if settings are not loaded yet
    const hapticEnabled = settings?.hapticBuzz ?? true;
    
    if (hapticEnabled && 'vibrate' in navigator && typeof navigator.vibrate === 'function') {
      try {
