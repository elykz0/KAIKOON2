...
export const useNotifications = () => {
  ...
  const showKaibloomsCelebration = useCallback((kaibloomsAwarded: number) => {
    if (!settings?.celebrationNotificationsEnabled) return;

    const title = 'Kaiblooms Earned! 🌸';
    const body = `You earned ${kaibloomsAwarded} Kaibloom${kaibloomsAwarded > 1 ? 's' : ''} for completing your task!`;
    showNotification(title, { body, type: 'celebration' });
  }, [settings?.celebrationNotificationsEnabled, showNotification]);
