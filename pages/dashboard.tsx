...
import { Plus, Settings, BarChart3, Leaf, Music, BookOpen } from 'lucide-react';
import { useTasks, useUserProgress } from '../helpers/useTaskQueries';
import { useSettings } from '../helpers/useSettingsQueries';
import { useNotifications } from '../helpers/useNotifications';
import { useCreateTaskFromJournal } from '../helpers/useTaskCreation';
...
const DashboardPage = () => {
  ...
  const [isJournalDialogOpen, setJournalDialogOpen] = useState(false);
  ...
  const { createMultipleTasks, isCreating } = useCreateTaskFromJournal();
  ...
  const handleJournalTasksCreated = async (tasks: Array<{ title: string; estimatedMinutes: number }>) => {
    if (tasks.length > 0) {
      ...
      {isLoading && !tasks && !userProgress ? (
        ...
        <div className={styles.container}>
          <header className={styles.header}>
            ...
            <div className={styles.headerRight}>
              ...
              <Button 
                variant="ghost" 
                size="icon" 
                aria-label="Journal & Task Analysis"
                className={styles.settingsButton}
          ...
          <JournalDialog
            ...
            onTasksCreated={handleJournalTasksCreated}
          />
