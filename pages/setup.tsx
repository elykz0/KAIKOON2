...
const SetupPage: React.FC = () => {
  ...
  const { settings, isLoadingSettings, completeSetup, isUpdatingSettings, error } = useSetupFlow();
  ...
  // Handle any errors from the setup flow
  if (error) {
    return (
      <div className={styles.container}>
        <div className={styles.card}>
          <div className={styles.header}>
            <h1 className={styles.title}>Setup Error</h1>
            <p className={styles.subtitle}>There was an error loading your setup. Please try refreshing the page.</p>
          </div>
        </div>
      </div>
    );
