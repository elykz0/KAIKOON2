...
const TaskDetailPage = () => {
  ...
  const { showCelebration, showKaibloomsCelebration } = useNotifications();
  ...
  const handleReflectionSuccess = () => {
    console.log('handleReflectionSuccess called');
    if (!task) {
      console.log('No task available, returning');
      return;
    }

    console.log('Reflection success - marking task as completed:', task.id);
    ...
    updateTaskMutation.mutate(
      ...
      {
        onSuccess: (data) => {
          console.log('Task update successful - navigating back to dashboard');
          if (task) {
            showCelebration(task.title); // Final celebration notification after reflection completion
          }
          
          // Show Kaiblooms celebration if awarded
          if (data.kaibloomsAwarded && data.kaibloomsAwarded > 0) {
            showKaibloomsCelebration(data.kaibloomsAwarded);
          }
